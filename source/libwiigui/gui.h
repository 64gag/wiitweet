 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 
 * WiiTweet: libwiigui was slightly modified by Pedro Aguiar, I state this so you
 * don't expect everything to work the way it works on the original libwiigui and
 * to protect tantric's reputation from some ugly code I threw around.
*/
...
#define MAX_KEYBOARD_DISPLAY	26
...
class GuiImageData
{
	public:
		...
		u8 GetFormat();
		u8 shared;	//times the image data is shared
		void Delete(); //Call this instead of the delete operator when the image data is shared
	protected:
...
class GuiText : public GuiElement
{
	public:
		...
		//!\param s Font size
		//!\param c Font color
		GuiText(const char * t = "", int s = 22, GXColor c = ((GXColor){0, 0, 0, 255}) );
		//!\overload
...
{
	public:
		GuiKeyboard(char * t, u32 m, int tw);
		~GuiKeyboard();
	...
	protected:
		...
		void UTF8_analize(const char * utf8);
		void Update(GuiTrigger * t);
		void ac_select(double roll, double pitch);
		GuiWindow * irKeyboard;
		GuiWindow * acKeyboard;
		u32 kbtextmaxlen;
		int CurrentKbIndex;
		int isIR;
		int shift;
		...
		char char_len_str[16];
		unsigned int mem_len;
		unsigned int cursor_char_pos;
		unsigned int cursor_mem_pos;
		u32 tweet;
		...
		GuiButton * ac_keyBtn[4][6];
		GuiImage * ac_keyImg[4][6];
		GuiImage * ac_keyImgOver[4][6];
		GuiText * ac_keyTxt[4][6];

		GuiImageData * keyTextbox;
		GuiImageData * key;
		GuiImageData * keyOver;
		GuiImageData * ac_key;
		GuiImageData * ac_keyOver;
...
/***************************************************************************
 GuiRiver is a gui container that inherits from GuiWindow.
 It differs from GuiWindow in that it handles the logic to easily display
 and browse elements in a stack-like fashion
****************************************************************************/

class GuiRiver : public GuiWindow{
	public:
		GuiRiver();
		~GuiRiver();
		void Append(GuiElement* e);
...
class GuiToolbar : public GuiWindow{
	public:
		...
		void Update(GuiTrigger * t);
		time_t timer;
		GuiImageData * ToolbarImgData;
		GuiImage * ToolbarImg;
		int animate;
};

class GuiInfomsg : public GuiWindow{
	public:
		GuiInfomsg();
		~GuiInfomsg();
		void Update(GuiTrigger * t);
