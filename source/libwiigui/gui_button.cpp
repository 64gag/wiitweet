...
{
	if(!parentElement->IsVisible() || state == STATE_CLICKED || !this->IsVisible() || state == STATE_DISABLED || !t)
		return;
	...
	if(this->IsClickable() || this->IsHoldable())
	{
		...
		else if(ac_button && parentKeyboard && (ac_button == ((GuiKeyboard *)parentKeyboard)->ac_selected))
		{
			if(state == STATE_DEFAULT) // we weren't on the button before!
			{
				this->SetState(STATE_SELECTED, t->chan);
	...
	if(updateCB && state == STATE_CLICKED)
		updateCB(this);
}
