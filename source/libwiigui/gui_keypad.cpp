...
GuiKeypad::GuiKeypad(char * t, u32 max)
{
	...
	kpText = new GuiText(GetDisplayText(kptextstr), 20, (GXColor){0, 0, 0, 0xff});
	...
	// disable '-' and '.' by default
	keyBtn[3][0]->SetState(STATE_DISABLED);
	keyBtn[3][0]->SetVisible(false);
	keyBtn[3][2]->SetState(STATE_DISABLED);
	keyBtn[3][2]->SetVisible(false);
}
...
void GuiKeypad::EnableNegative()
{
	...
	keyBtn[3][2]->SetState(STATE_DEFAULT);
	keyBtn[3][2]->SetVisible(true);
}
