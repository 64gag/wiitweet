...
{
	m_oAuth.setConsumerKey(std::string("6gyPp3PB9ISEdsekGpCBYw"));
	m_oAuth.setConsumerSecret(std::string("8ivcRzWzhdrwkX9CpVgxwhZ8da7DO3SkgFSMzq72thI"));
...
*
*--*/
bool twitCurl::search( std::string& searchQuery, std::string& max_id )
{
    /* Prepare URL */
    std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[twitCurlTypes::eTwitCurlProtocolHttp] +
                           twitterDefaults::TWITCURL_SEARCH_URL +
                           twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[twitCurlTypes::eTwitCurlApiFormatAtom] +
                           twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_SEARCHQUERYSTRING +
                           urlencode(searchQuery) + twitCurlDefaults::TWITCURL_URL_SEP_AMP + "rpp=20" +
                           twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_INCENTITIES;
			if(max_id.length()){
				buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_MAXID + max_id;
			}

    /* Perform GET */
    return performGet( buildUrl, 1 );
}
...
*
*--*/
bool twitCurl::statusUpdate( std::string& newStatus, std::string * replyTo )
{
    bool retVal = false;
    if( newStatus.length() )
    {
	...
	if(replyTo){
		newStatusMsg += twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_INREPLYTO + (*replyTo);
	}
        /* Perform POST */
...
bool twitCurl::statusShowById( std::string& statusId )
{
    ...
    if( statusId.length() )
    {
        ...
        std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                               ...
                               twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType]+
                               twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_COMMON_PARAMS;
...
bool twitCurl::usersSearch( std::string& searchQuery, std::string& page)
{

    /* Prepare URL */
    std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                           twitterDefaults::TWITCURL_SEARCHUSERS_URL +
                           twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType] +
                           twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_SEARCHQUERYSTRING + urlencode(searchQuery) +
	                    twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_COMMON_PARAMS +
       	             twitCurlDefaults::TWITCURL_URL_SEP_AMP + "per_page=20";
       	             
       	             if(page != "0"){ buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP + "page=" + page; }
...
*
* @description: method to get the home timeline
*
* @input: none
*
* @output: true if GET is success, otherwise false. This does not check http
...
*
*--*/
bool twitCurl::timelineFriendsGet(std::string& max_id)
{
...
*
*--*/
bool twitCurl::mentionsGet( std::string &max_id )
{
    std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
				twitterDefaults::TWITCURL_MENTIONS_URL +
				twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType];

	buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_COMMON_PARAMS;
...
*
*--*/
bool twitCurl::timelineUserGet( bool trimUser, unsigned int tweetCount, std::string& max_id, std::string userInfo, bool isUserId )
{
    /* Prepare URL */
    std::string buildUrl;
...
bool twitCurl::friendsIdsGet( std::string& userInfo, bool isUserId )
{
    ...
    utilMakeUrlForUser( buildUrl, twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                        ...
                        twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType],
                        userInfo, isUserId );
		buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP + "cursor=-1";
    /* Perform GET */
    return performGet( buildUrl );
}

bool twitCurl::usersLookup( std::string& userInfo, bool isUserId)
{
    /* Prepare URL */
    std::string buildUrl;
    utilMakeUrlForUser( buildUrl, twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                        twitterDefaults::TWITCURL_USERSLOOKUP_URL +
                        twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType],
                        userInfo, isUserId );
...
bool twitCurl::friendshipExists( std::string& user_id_a, std::string& user_id_b)
{
    std::string buildUrl;

	buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                        twitterDefaults::TWITCURL_FRIENDHISPEXISTS_URL +
                        twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType];
	buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_QUES + "user_id_a=" +user_id_a + twitCurlDefaults::TWITCURL_URL_SEP_AMP + "user_id_b=" +user_id_b ;
    
	if (performGet( buildUrl ) ){

		std::string response;
		getLastWebResponse( response );

		if(response.find("<friends>true</friends>") != std::string::npos){
			return true;
		}
	}
...
*
*--*/
bool twitCurl::favoriteGet( std::string& userInfo, bool isUserId, std::string& max_id )
{
...
bool twitCurl::retweet( std::string& statusId )
{
    /* Prepare URL */
    std::string buildUrl = twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
...
*
*--*/
bool twitCurl::performGet( const std::string& getUrl, int anon )
{
...
void utilMakeUrlForUser( std::string& outUrl, const std::string& baseUrl, std::string& userInfo, bool isUserId )
{
    ...
    if( userInfo.length() )
    {
        ...
        outUrl += userInfo + twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_COMMON_PARAMS;
    }   
}
